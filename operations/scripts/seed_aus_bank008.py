...
seed_aus_bank008.py
───────────────────
Adds Australia as a new jurisdiction and seeds BANK008 (Southern Cross Bank Ltd.)
with 50 initial customers.

Steps:
  1. Insert AUS into countries + country_macro (CY2024 + CY2025)
  2. Insert BANK008 master row + branch
  3. Seed 50 customers with full records
     (customers / customer_kyc / accounts / opening deposit txn /
      loans / credit_risk_metrics / bank_loan_metrics)
Idempotent: each section skips if its data already exists.
"""
...
BANK_ID   = 'BANK008'
BANK_NAME = 'Southern Cross Bank Ltd.'
BRANCH_ID = 'BR-SCROSS-001'
IFSC      = 'SCRX0000001'
N_CUSTOMERS = 50

FIRST_NAMES = [
    'James', 'William', 'Oliver', 'Noah', 'Jack', 'Liam', 'Ethan', 'Lucas', 'Henry', 'Thomas',
    'Charlotte', 'Olivia', 'Amelia', 'Isla', 'Sophie', 'Grace', 'Chloe', 'Hannah', 'Emily', 'Zoe',
]
LAST_NAMES = [
    'Smith', 'Jones', 'Williams', 'Brown', 'Taylor', 'Wilson', 'Anderson',
...
PRINCIPAL_RANGE = (2_200_000, 6_000_000)
BALANCE_RANGE   = (1_500_000, 4_200_000)
...
def main():
    ...
    # ── 1. Country ─────────────────────────────────────────────────────────
    cur.execute("SELECT country_code FROM countries WHERE country_code='AUS'")
    if not cur.fetchone():
        cur.execute(
            "INSERT INTO countries (country_code, country_name, region, sub_region, "
            "currency_code, currency_symbol, central_bank, central_bank_abbr, "
            "capital_regulator, basel_framework, sovereign_rating, "
            "min_crar, min_cet1, min_tier1, min_lcr, min_nsfr, is_home, generated_at) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            ('AUS', 'Australia', 'Asia-Pacific', 'Oceania',
             'AUD', 'A$', 'Reserve Bank of Australia', 'RBA',
             'APRA', 'Basel III (APRA CPS 110)', 'AAA',
             10.5, 7.0, 8.5, 100.0, 100.0, 0, now)
        )
        print("  Created country: AUS — Australia (AAA, Asia-Pacific/Oceania)")
    else:
        print("  AUS already in countries — skipping.")

    # ── 2. Country macro ───────────────────────────────────────────────────
    macro_rows = [
        ('AUS', 'CY2024', 1790.0, 1.5, 3.8, 4.35, 4.0, 36.0, -1.5, 1.53, 26.5),
        ('AUS', 'CY2025', 1855.0, 2.1, 3.0, 3.85, 4.2, 35.5, -1.2, 1.56, 26.8),
    ]
    for row in macro_rows:
        cur.execute("SELECT id FROM country_macro WHERE country_code=? AND period=?",
                    (row[0], row[1]))
        if not cur.fetchone():
            cur.execute(
                ...
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (*row, 'reference data (RBA/ABS/IMF, illustrative)', now)
            )
            print(f"  Added macro row: AUS {row[1]} — GDP growth {row[2]}%, policy rate {row[4]}%")
    ...
    if not cur.fetchone():
        cur.execute(
            ...
            (BANK_ID, BANK_NAME, 'SCX008', 'Australia', 'Sydney', 'New South Wales',
             1985, 'Active', 'AUS')
        )
        print(f"  Created bank master row: {BANK_ID} — {BANK_NAME}")
    else:
        print(f"  {BANK_ID} already exists — skipping master row.")
    ...
    if not cur.fetchone():
        cur.execute(
            ...
            "VALUES (?,?,?,?,?,?,?)",
            (BRANCH_ID, BANK_ID, 'Southern Cross Bank — Sydney Main',
             'Sydney', 'New South Wales', IFSC, 'Active')
        )

    # ── 5. Customers ───────────────────────────────────────────────────────
    cur.execute("SELECT COUNT(*) FROM customers WHERE bank_id=?", (BANK_ID,))
    if cur.fetchone()[0] > 0:
        print(f"  {BANK_ID} already has customers — skipping seed.")
        conn.commit()
        conn.close()
        return
    ...
    for i in range(N_CUSTOMERS):
        ...
        if good:
            de = round(random.uniform(0.4, 1.6), 2);  ic = round(random.uniform(4.5, 13.0), 2)
            ...
            late = 0;  prev_def = 0;                    income = random.randint(900_000, 5_000_000)
        else:
            ...
            cibil = random.randint(540, 680);           foir = round(random.uniform(0.50, 0.88), 2)
        ...
        rate        = round(random.uniform(8.5, 15.0), 2)
        tenure      = random.choice([36, 48, 60, 84, 120, 180])
        loan_emi_v  = emi(principal, rate, tenure)
        ...
        lid     = f"SCROSS-LN-{idx:05d}-N{idx}"
        aid     = f"ACC-SCROSS-{idx:04d}"
        ...
        cur.execute(
            ...
            (cid, BANK_ID, first, last, f"{1966+(60-age)}-06-15",
             ...
             f"+61-{random.randint(2,9)}-{random.randint(1000,9999)}-{random.randint(1000,9999)}",
             f"{random.randint(1,99)} Collins Street", city, city,
             str(random.randint(2000, 6999)),
        ...
        cur.execute(
            ...
            (lid, BANK_ID, cid, ltype, principal, rate, tenure, loan_emi_v, disb, mat,
        ...
        cur.execute(
            ...
            (cid, BANK_ID, 1, 1, 'Verified', '2024-01-05', age,
             ...
             'Southern Cross Corp', 'Financial Services', years_emp, income, 0, foir,
        ...
        cur.execute(
            ...
            (BANK_ID, BANK_NAME, lid, de, ic, pm, lq, default_flag, pd_obs, obs, now,
             ...
             cibil, prev_def, months_cust, late, ex_loans, ex_products, 0, 'AUS'))
