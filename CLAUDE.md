...
| Phase 5.19 | **Financial Reporting data completeness fix + PD model retrain on real customers.** `seed_bank_profit_loss.py` was only populating BANK007; ran it for all 9 banks so every bank now has FY2024 + FY2025 P&L alongside Balance Sheet, Key Ratios, and Pillar 3 — Financial Reporting & Disclosures dashboard is now fully populated for all 9 banks and all regional/country/consolidated roll-ups. PD model retrained on 1,166 real-bank customer rows exported from `bank_loan_metrics` (all 9 banks, 932 train / 234 test); model promoted; peer benchmarks updated from the same corpus. | June 19, 2026 |
