...
**Global group hierarchy (June 17, 2026):** the platform is now a **global group bank** organised as **Group → Region → Country → Bank**. Banks map to a country (`banks.country_code`); countries carry region/currency/regulator + macro variables (`countries` + `country_macro` tables). The two original India banks (BANK001/002) plus four foreign group banks (BANK003 Atlas Bank N.A./USA, BANK004 Britannia Banking Group/UK, BANK005 Lion City Bank/Singapore, BANK006 Gulf Union Bank/UAE) span 4 regions / 5 countries. **All six banks are grounded in a real ledger** (Phase 5.17): each foreign bank has its own customers/loans/accounts/transactions (generated by `seed_global_customers.py`, ~86 foreign customers total), so its balance sheet `advances_net` == `SUM(loans.outstanding)` and deposits == `SUM(accounts.balance)`, and CAR/LCR/GNPA/P&L all roll up from the per-loan ledger — identical integrity to the India banks. Foreign ledgers are denominated in the **group reporting currency (₹)**. Financial Reporting rolls up at every level (region & country aggregates reuse `financial_reports.consolidate()`). The `regulatory_engine` no-loan RWA fallback remains only as a safety net for genuinely empty banks; the foreign banks now use the normal per-loan path.
...
- **Seed:** `operations/scripts/seed_global.py` — creates/populates `countries` + `country_macro`, adds `banks.country_code`, inserts the foreign **bank-master rows**, a few foreign `regulatory_bodies` rows, then calls `seed_global_customers.seed()` to ground them. Idempotent. Self-seeded via `app._ensure_global` (called **before** the BS/P&L seed inside `_ensure_financials`, so the foreign loans exist when the sheets are live-anchored).
...
| Phase 5.17 | **Grounded the foreign banks** — `seed_global_customers.py` generates a real ledger (customers/loans/accounts/txns/credit_risk_metrics/bank_loan_metrics) for BANK003–006 so their balance sheet/P&L/CAR/LCR/GNPA roll up from actual loans (parity with the India banks; ~86 foreign customers, group ₹). `seed_global.py` no longer hardcodes foreign BS/P&L; `_ensure_financials` seeds the global layer before BS/P&L; BS/P&L seeders skip-guard now naturally includes the foreign banks. PD model corpus (`bank_loan_metrics`) now spans all 6 banks | June 17, 2026 |
